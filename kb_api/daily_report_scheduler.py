...
def _scheduled_time() -> tuple[int, int]:
    raw = str(getattr(settings, "daily_report_run_time", "00:10"))
    try:
...
def _status_path(root_dir: Path | str) -> Path:
    return Path(root_dir) / "operations" / "daily_report_automation.json"
...
def _status_default() -> dict:
    return {
        ...
        "check_interval_seconds": int(getattr(settings, "daily_report_check_interval_seconds", 60)),
...
def load_daily_report_automation_status(root_dir: Path | str) -> dict:
    return _read_status(_status_path(root_dir))


def _save_daily_report_automation_status(root_dir: Path | str, payload: dict) -> dict:
...
def run_daily_report_automation(root_dir: Path | str, report_date: str | None = None, force: bool = False) -> dict:
    root_dir = Path(root_dir)
    now = _now()
    status = load_daily_report_automation_status(root_dir)
    ...
    _save_daily_report_automation_status(root_dir, status)
    ...
    try:
        for day in pending:
            ...
            append_operation_event(
                root_dir,
                event_type="daily_report_auto_trigger",
                knowledge_base_id="platform_run_memory",
                source="scheduler/daily-report",
                actor="system",
                params={"report_date": day, "force": force},
                status="running",
                remark="daily report auto trigger",
            )
            payload = ingest_daily_report_to_memory(root_dir, event_date=day, save_report=True)
            ...
            append_operation_event(
                root_dir,
                event_type="daily_report_auto_success",
                knowledge_base_id="platform_run_memory",
                source="scheduler/daily-report",
                actor="system",
                params={"report_date": day, "report_path": payload.get("report_path", "")},
                status="success",
                remark="daily report auto success",
            )
        ...
        append_operation_event(
            root_dir,
            event_type="daily_report_auto_failed",
            knowledge_base_id="platform_run_memory",
            source="scheduler/daily-report",
            actor="system",
            ...
            remark="daily report auto failed",
        )
        raise


def _scheduler_loop(root_dir: Path | str) -> None:
    interval = max(15, int(getattr(settings, "daily_report_check_interval_seconds", 60)))
    while True:
        try:
            now = _now()
            status = load_daily_report_automation_status(root_dir)
            ...
            _save_daily_report_automation_status(root_dir, status)
            if pending:
                run_daily_report_automation(root_dir)
        except Exception:
            pass
        time.sleep(interval)


def ensure_daily_report_scheduler_started(root_dir: Path | str) -> dict:
